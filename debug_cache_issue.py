#!/usr/bin/env python3
"""
Debug script to test the data manager cache issue
"""
import pandas as pd
import numpy as np
import sys
...
def debug_cache_issue():
    ...
    try:
        ...
        # Get indicators (should calculate fresh)
        print("📈 Getting fresh indicators...")
        ...
        if indicators:
            ...
            # Now test the cache by calling again
            print("🔄 Testing cache by calling again...")
            cached_indicators = data_manager.calculate_indicators_for_ticker(test_ticker)
            
            if cached_indicators:
                print(f"✅ Cached indicators retrieved: {len(cached_indicators)}")
                
                # Check if cached indicators are the same
                print("🔍 Checking cache consistency...")
                for sma_key in ['sma_21', 'sma_50', 'sma_200']:
                    if sma_key in indicators and sma_key in cached_indicators:
                        original = indicators[sma_key]
                        cached = cached_indicators[sma_key]
                        if hasattr(original, 'equals') and hasattr(cached, 'equals'):
                            if original.equals(cached):
                                print(f"✅ {sma_key} cache is consistent")
                            else:
                                print(f"⚠️  {sma_key} cache is inconsistent!")
                        else:
                            print(f"⚠️  Cannot compare {sma_key} cache")
                print()
